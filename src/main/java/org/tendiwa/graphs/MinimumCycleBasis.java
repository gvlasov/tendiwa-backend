...
public class MinimumCycleBasis<V, E> {
    ...
    private final PrimitiveContainer<V> primitives = new PrimitiveContainer<>();
    ...
    private void extractPrimitive(V v0) {
        ...
        if (vcurr == null) {
            ...
            if (graph.containsVertex(v1) && graph.degreeOf(v1) == 1) {
                ...
                extractFilament(v1, neighborIndex.neighborListOf(v1).get(1));
            }
        } else {
    ...
    private V getMost(V vprev, V vcurr, boolean clockwise) {
        if (neighborIndex.neighborsOf(vcurr).isEmpty()) {
            return null;
        }
        double[] dcurr;
        if (vprev == null) {
...
//                assert false : "Think about it again";
     ...
     * @param v2
     *         Another vector.
     * @return Perp dot product of vectors v1 and v2.
     * @see <a href="http://mathworld.wolfram.com/PerpDotProduct.html">Perp Dot Product</a>
     */
    private double perpDotProduct(double[] v1, double[] v2) {
