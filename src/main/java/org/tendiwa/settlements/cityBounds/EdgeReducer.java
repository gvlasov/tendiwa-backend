...
final class EdgeReducer {
	...
	private static Collection<Cell> findIntersectionCells(BiMap<Cell, Point2D> map) {
		...
		for (Cell cell : map.keySet()) {
			...
			for (Direction dir : CardinalDirection.values()) {
				if (map.containsKey(cell.moveToSide(dir, 1))) {
					neighbourCells++;
				}
			}
	 ...
	 * @param map
	 * 	A mapping to vertices of {@code graphToMutate} from those vertices transformed to {@link org.tendiwa.geometry.Cell}s.
