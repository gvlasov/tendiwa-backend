...
public final class BuildingsTouchingStreets {
	...
	private final Map<List<Point2D>, Set<RectangleWithNeighbors>> streetsToPlaces = new LinkedHashMap<>();
	private final Map<RectangleWithNeighbors, Set<List<Point2D>>> placesToStreets = new LinkedHashMap<>();
	 ...
	 */
	public void addBuilding(RectangleWithNeighbors lot) {
	...
	private Set<List<Point2D>> collectStreetsForBuildingPlace(RectangleWithNeighbors buildingPlace) {
	...
	Set<RectangleWithNeighbors> getPlacesOnStreet(Street street) {
		return Collections.unmodifiableSet(streetsToPlaces.get(street));
	}

	Set<List<Point2D>> getStreetsForBuildingPlace(RectangleWithNeighbors where) {
