 ...
 * constructing a {@link PathGeometry}.
 */
public final class RoadRejector {
	 ...
	 *
	 * @param graph
	 * 	A full road graph ({@link PathGeometry#getFullRoadGraph()}) of {@code cityGeometry}.
	 * @param pathGeometry
	...
	public static UndirectedGraph<Point2D, Segment2D> rejectAllCellsBorders(
		...
		PathGeometry pathGeometry // TODO: Maybe extract an interface with cycle() method so graph and cityGeometry won't seem coupled
	) {
		UndirectedSubgraph<Point2D, Segment2D> modifiedGraph = new UndirectedSubgraph<>(graph, graph.vertexSet(), graph.edgeSet());
		for (NetworkWithinCycle networkWithinCycle : pathGeometry.getNetworks()) {
	...
	public static UndirectedGraph<Point2D, Segment2D> rejectPartOfNetworksBorders(
		...
		return new RoadRejector(graph, random).rejectRoads(pathGeometry, probability);
	}
	...
	private UndirectedGraph<Point2D, Segment2D> rejectRoads(
		...
		List<List<Segment2D>> chains = findChainsBetween3DegreeCycleVertices(pathGeometry);
	...
	private List<List<Segment2D>> findChainsBetween3DegreeCycleVertices(PathGeometry pathGeometry) {
		List<List<Segment2D>> answer = new ArrayList<>();
		for (UndirectedGraph<Point2D, Segment2D> outerCycleEdges : pathGeometry.outerCycleEdges().values()) {
	 ...
	 *
	 * @see PathGeometry#outerCycleEdges() for what kind of subgraph is walked along.
	 */
