...
public class RectangleSidePiece {
	...
	/**
	 * Creates a rectangle out of this RectangleSidePiece, such that this piece forms one side of a rectangle, and
	 * another
	...
	public Cell getCellInside(CardinalDirection side, int depth) {
		if (side.getOrientation() == direction.getOrientation()) {
			throw new IllegalArgumentException();
		}
		switch (side) {
			case N:
				return new Cell(segment.getX(), segment.getY() + depth);
			case W:
				return new Cell(segment.getX() + depth, segment.getY());
			case E:
				return new Cell(segment.getX() + segment.getLength() - 1 - depth, segment.getY());
			case S:
				return new Cell(segment.getX(), segment.getY() + segment.getLength() - 1 - depth);
			default:
