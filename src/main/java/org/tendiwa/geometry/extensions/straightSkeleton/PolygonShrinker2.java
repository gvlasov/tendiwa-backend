...
public class PolygonShrinker2 {

	private final Set<Segment2D> shrunkPolygonsSegments;
	...
	PolygonShrinker2(
		...
		Map<Segment2D, MinimalCycle<Point2D, Segment2D>> edgesToFaces = mapOriginalEdgesToFaces(graph, edges);
		shrunkPolygonsSegments = findShrunkPolygonsSegments(depth, edgesToFaces);
	}

	UndirectedGraph<Point2D, Segment2D> asGraph() {
	...
	private Set<Segment2D> findShrunkPolygonsSegments(
		double depth,
		Map<Segment2D, MinimalCycle<Point2D, Segment2D>> edgesToFaces
	) {
		ApproximatedEdges edges = new ApproximatedEdges();
		for (Map.Entry<Segment2D, MinimalCycle<Point2D, Segment2D>> e : edgesToFaces.entrySet()) {
			...
			while (!queue.isEmpty()) {
				edges.addFixedEdge(new Segment2D(
					...
					queue.poll().getIntersectionPoint(),
					queue.poll().getIntersectionPoint()
				));
			}
		}
	...
	private Map<Segment2D, MinimalCycle<Point2D, Segment2D>> mapOriginalEdgesToFaces(
		...
		Map<Segment2D, MinimalCycle<Point2D, Segment2D>> edgeToFace = new HashMap<>();
	...
	private UndirectedGraph<Point2D, Segment2D> fillNewGraphWithArcsAndEdges(
		Multimap<Point2D, Point2D> arcs,
		List<Segment2D> edges
	) {
		UndirectedGraph<Point2D, Segment2D> graph = new SimpleGraph<>(Segment2D::new);
