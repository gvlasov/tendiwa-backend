...
class CityBoundsWithHoles implements Runnable {
	...
	TestCanvas canvas = new TestCanvas(3, worldRec.width, worldRec.height);
	...
	public void run() {
		...
		AtomicInteger counter = new AtomicInteger(110);
		...
		for (int i = 0; i < iterations; i++) {
			...
			canvas.draw(cityBounds, DrawingGraph.withColorAndVertexSize(Color.black, 1));
	...
	private UndirectedGraph<Point2D, Segment2D> buildCityBoundsGraph(CellSet water) {
		...
		return new CityBoundsFactory(water).create(
			cityShape,
			startCell,
			cityRadius
		);
