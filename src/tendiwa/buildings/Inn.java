...
public class Inn extends Building {
	...
	public void draw() {
		...
		RectangleArea leftRooms = crs.cutRectangleFromSide(rightRooms, side.clockwiseQuarter(), ((((dir.isHorizontal()) ? width
			: height) - lobbyWidth) / 2 - 1));
		// Separate middle rectangle (lobby) and right rectangle, get lobby
		RectangleArea lobby = crs.cutRectangleFromSide(rightRooms, side.clockwiseQuarter(), lobbyWidth);
		
		// Separate rectangle above lobby
		RectangleArea aboveLobby = crs.cutRectangleFromSide(lobby, side.opposite(), 4);
		// Left hall
		RectangleArea leftHall = crs.cutRectangleFromSide(leftRooms, side.counterClockwiseQuarter(), 2);
		// Right hall
		RectangleArea rightHall = crs.cutRectangleFromSide(rightRooms, side.clockwiseQuarter(), 2);
