...
/**
 * <p> Compares same-direction pieces. If two pieces lie on different lines, the greatest is the one that lies further
 * in direction they are facing. If two pieces lie on the same lie, the greatest is the one that has greater dynamic
 * coordinate. </p>
 */
...
class VirtualWave {
	...
	/**
	 * <p> Checks if two perpendicular pieces are close enough to be rectangleable. </p> <p> Meeting this condition is not
	 * enough to be able to placeIn a rectangle between these two pieces: you'd also need to ckeck whether there are pieces
	...
	class RectangleablePiecesCollection {
		...
		/**
		 * Checks if there are pieces that bother placing rectangle in junction between two parallel pieces whose dynamic
		 * ranges don't intersect.
		 *
		 * @param piece
		 * 		A piece parallel to {@code parallelPiece} whose dynamic range doesn't intersect that piece range.
		 * @param parallelPiece
		 * @return
		 */
		...
		/**
		 * Checks if other pieces between {@code piece} and {@code parallelPiece} bother placing a rectangle between those two
		...
		/**
		 * Returns a minimal possible rectange that must not intersect any pieces in order for argument pieces to be
		 * rectangleable.
		 *
		 * @param pieces
		 * @return
		...
		/**
		 * <p> Checks if two pieces can be joined into one. They can, if they: <ol> <li>Face the same direction</li> <li>Lie
		 * on equal lines</li> <li>There is amount of cells between their closest ends equal to {@link
		 * RectangleSystem#borderWidth}. </ol> </p> <p/> <p> If this method returns true, then these pieces can be passed to
		 * {@link RectangleablePiecesCollection#joinPieces(RectangleSidePiece, RectangleSidePiece)} </p>
		 *
		 * @param piece1
		 * 		A piece.
		 * @param piece2
		...
		/**
		 * Disassociates a piece with a junction containing it. Each piece is a part of only one junction, so no need to
		...
		class PerpendicularPiecesPair implements
			...
			/**
			 * Returns lesser piece of this pair if {@code pieceFromAnotherPair} is lesser in {@code anotherPair} . Returns
			 * greater piece of this pair if {@code pieceFromAnotherPair} is greater in {@code anotherPair}. Returns null if this
